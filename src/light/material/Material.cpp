...
// 本节就是把 物体颜色 和 光照颜色 在冯模型下, 都分别拆分为三个量, 环境量, 漫反射量, 镜面反射量, 物体的多一个镜面反射系数,用来调节反射强度
int Material::run()
{
	// settings
	...
	Shader object_shader("src/light/material/vertex.shader", "src/light/material/fragment.shader");
	Shader light_shader("src/light/material/vertex.shader", "src/light/material/light_fragment.shader");
	...
	while (!glfwWindowShouldClose(window))
	{
...

		{
			...
			model_matrix = glm::scale(model_matrix, glm::vec3(4, 4, 4));
			...
			// 设置物体材质
			object_shader.set_vec3("material.environment_color", new glm::vec3(1.0f, 0.5f, 0.31f));
			object_shader.set_vec3("material.diffuse_color", new glm::vec3(1.0f, 0.5f, 0.31f));
			object_shader.set_vec3("material.mirror_color", new glm::vec3(0.5f, 0.5f, 0.5f));
			...
			object_shader.set_vec3("light.environment_color", &ambientColor);
			object_shader.set_vec3("light.diffuse_color", &diffuseColor);
